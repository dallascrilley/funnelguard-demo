// Funnelguard — rule engine
// All rules are pure functions: (FunnelConfig) => Finding[]
// No async, no API, no LLM. Deterministic and composable.

import type { FunnelConfig, Finding, Rule } from './types.ts';

// ─── UTM Integrity Rules ─────────────────────────────────────────────────────

const checkRetiredCampaignReferences: Rule = (config) => {
...
const checkUTMMediumTaxonomy: Rule = (config) => {
  const findings: Finding[] = [];
...
const checkMissingUTMCampaign: Rule = (config) => {
  const findings: Finding[] = [];
...
const checkOrphanedFormBindings: Rule = (config) => {
  const findings: Finding[] = [];
...
const checkFormWithNoOwnerAssignment: Rule = (config) => {
  const findings: Finding[] = [];
...
const checkFormIDMismatch: Rule = (config) => {
  const findings: Finding[] = [];
...
const checkLifecycleDowngrade: Rule = (config) => {
  const findings: Finding[] = [];
...
const checkUnownedMQLs: Rule = (config) => {
  const findings: Finding[] = [];
...
const checkCustomerInLeadNurture: Rule = (config) => {
  const findings: Finding[] = [];
...
const checkStaleSQL: Rule = (config) => {
  const findings: Finding[] = [];
...
const checkRetiredCampaignAttribution: Rule = (config) => {
  const findings: Finding[] = [];
  const { retiredCampaigns } = config.utmTaxonomy;
...
const checkOrphanedWorkflows: Rule = (config) => {
  const findings: Finding[] = [];
  const { retiredCampaigns } = config.utmTaxonomy;
...
const checkMissingOwnerAssignEnrollment: Rule = (config) => {
  const findings: Finding[] = [];
